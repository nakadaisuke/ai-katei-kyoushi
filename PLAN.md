...
小学4年生の全15単元（下表）も同じ方針で、**宮城県教育センター**「わくわくワーク」小学4年算数（`https://www.edu-c.pref.miyagi.jp/tangen/math/ele4_math/answer/*.els4.kaitou.*.pdf`、ホップ・ステップ・ジャンプの3段階×15単元、教員向け解答付き）の設問・解答のみを出典として作成した（各ファイル冒頭のコメントに出典URLを明記）。今回は宮城県教材のみで各単元11〜25問を確保できたため、埼玉県・大阪府の教材（両サイトとも小4セクションが存在することは確認済み）は未使用。

| ファイル | 単元 | 問題数 |
|---|---|---|
| `g4-large-numbers.ts` | 大きい数のしくみ | 21 |
| `g4-line-graphs.ts` | 折れ線グラフと表 | 22 |
| `g4-division-written-1.ts` | わり算の筆算(1) | 20 |
| `g4-angles.ts` | 角の大きさ | 20 |
| `g4-decimals.ts` | 小数のしくみ | 23 |
| `g4-soroban.ts` | そろばん | 13 |
| `g4-division-written-2.ts` | わり算の筆算(2) | 19 |
| `g4-rounding.ts` | がい数の使い方と表し方 | 25 |
| `g4-calculation-rules.ts` | 計算のきまり | 23 |
| `g4-quadrilaterals.ts` | 垂直，平行と四角形 | 17 |
| `g4-fractions.ts` | 分数 | 23 |
| `g4-function-tables.ts` | 変わり方調べ | 19 |
| `g4-area.ts` | 面積のはかり方と表し方 | 22 |
| `g4-decimal-multiplication-division.ts` | 小数のかけ算とわり算 | 25 |
| `g4-rectangular-prism.ts` | 直方体と立方体 | 11 |

`g4-soroban.ts`と`g4-rectangular-prism.ts`は他の単元より問題数が少ない。そろばんは珠の動きを絵で読み取る問題が中心、直方体と立方体は展開図の組み立てや見取図の作図が中心で、どちらも現状のアプリにはその図（そろばん・展開図・見取図）を表示する専用コンポーネントが無いため、文字だけで答えられる用語・計算問題にしぼって出題している（そろばんは軽い単元として作る方針をユーザーに確認済み）。同様に、角の大きさ（分度器の図）・垂直平行と四角形（斜めの図形や地図）・面積（階段状の図形）・折れ線グラフ（グラフそのもの、`table`図で代用）も、図が必須な設問は割愛または文章のみで解ける形に置き換えている。これら図コンポーネントの追加は今後の拡張課題とする。

今後、5年生以降の単元を追加する際も、AIに問題を自由生成させるのではなく、教科書・教育委員会等が公開する実在の問題を出典付きで採用する方針とする。ただし出典が商用利用不可のサイトの場合は、上記と同様に注記し、配布前に見直す。
